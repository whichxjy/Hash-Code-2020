...
int main(int argc, char* argv[]) {
    ...
    auto lib_cmp = [&](const Library& lhs, const Library& rhs) -> bool {
        ...
        while (!lhs_books.empty() && !rhs_books.empty()) {
            // ignore selected book ids
            while (!lhs_books.empty() && selected_book_ids.find(lhs_books.top().index) != selected_book_ids.end()) {
                lhs_books.pop();
            }
            if (lhs_books.empty()) {
                break;
            ...
            while (!rhs_books.empty() && selected_book_ids.find(rhs_books.top().index) != selected_book_ids.end()) {
                rhs_books.pop();
            }
            if (rhs_books.empty()) {
                break;
            ...
            lhs_sum += lhs_books.top().score;
            rhs_sum += rhs_books.top().score;
        ...
        else if (lhs.signup_day != rhs.signup_day) {
            // min
